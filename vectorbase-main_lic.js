...
// КОНФИГУРАЦИЯ WORKER
// ============================================
const WORKER_URL = 'https://vectorbase-protection.evgenstrizh.workers.dev';

// ============================================
// LICENSE VERIFICATION SYSTEM (ВЫПОЛНЯЕТСЯ СРАЗУ!)
// ============================================
// Promise который будет resolved когда лицензия проверена
window.LICENSE_VERIFICATION_PROMISE = (async function() {
    ...
    if (!LICENSE_KEY) {
        ...
        // Если body еще не загрузился, ждем
        if (!document.body) {
    ...
    console.log('🔑 License:', LICENSE_KEY.substring(0, 20) + '...');

    // Ждем пока body загрузится (если еще не загрузился)
    if (!document.body) {
        await new Promise(resolve => {
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', resolve, { once: true });
    ...
    try {
        ...
        console.log('📦 Client:', result.client_id || 'Unknown');
        ...
        if (!window.LICENSE_INFO || !window.LICENSE_INFO.valid) {
            ...
            showLicenseError('Connection Error', 'Unable to verify license. Check your internet connection.');
            throw error;
        }
    }
})();
    ...
    document.body.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#1a1a1a;color:#fff;font-family:Arial,sans-serif;text-align:center;padding:20px;">
            <div>
                ...
                <p style="font-size:14px;margin-top:30px;color:#666;">Domain: ${window.location.hostname}</p>
            </div>
        </div>
    `;
}
...
// Получение токена для запросов к Worker API
...
// Вызов Worker API с автоматической проверкой токена
async function callWorkerAPI(endpoint, data) {
    ...
    if (!response.ok) {
        if (response.status === 401) {
            showLicenseError('Session Expired', 'Please refresh the page');
...
document.addEventListener('DOMContentLoaded', async function() {
    // ===== ПРОВЕРКА ЛИЦЕНЗИИ (ПЕРВАЯ ПРОВЕРКА) =====
    // Ждем завершения проверки лицензии из IIFE
    console.log('⏳ DOMContentLoaded: Waiting for license verification to complete...');

    try {
        await window.LICENSE_VERIFICATION_PROMISE;
    } catch (error) {
        console.error('❌ License verification failed:', error);
        return; // Блокируем если лицензия невалидна
    }

    // Проверяем результат
    if (!window.LICENSE_INFO || !window.LICENSE_INFO.valid) {
        console.error('❌ License not valid - blocking interface');
        return; // Блокируем загрузку если лицензия невалидна
    }

    console.log('✅ License check passed, proceeding to JWT authentication');

    // ===== ИНИЦИАЛИЗАЦИЯ JWT АУТЕНТИФИКАЦИИ (ВТОРАЯ ПРОВЕРКА) =====
    ...
    // Инициализируем после успешной лицензии И аутентификации
...
// Парсинг записей (ЗАЩИЩЕНО - использует Worker)
async function parseRecords(text) {
    try {
        const result = await callWorkerAPI('/api/parse-records', { text: text });
        return result.records;
    } catch (error) {
        console.error('Ошибка парсинга записей:', error);
        showToast('Ошибка обработки данных', 'error');
        return [];
    }
}
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.formatted_text) {
                records = await parseRecords(data.formatted_text);
...
async function loadMonitoringData() {
    try {
        ...
        if (data.success) {
            ...
            await updateMonitoringDashboard(monitoringData);
            updateAnalyticsDashboard(monitoringData);
        } else {
...
async function useDemoMonitoringData() {
    monitoringData = {
        totalCycles: 8,
        totalDialogs: 120,
        approvedUpdates: 18,
...
async function updateMonitoringDashboard(data) {
...
if (typeCtx) {
    ...
    const typeStats = await calculateTypeStats(data.recentUpdates || []);
...
// Расчет статистики по типам (ЗАЩИЩЕНО - использует Worker)
async function calculateTypeStats(updates) {
    try {
        const result = await callWorkerAPI('/api/calculate-stats', { updates: updates });
        ...
        return {
            [t('monitoring.charts.addAction')]: 0,
            [t('monitoring.charts.editAction')]: 0,
            [t('monitoring.charts.deleteAction')]: 0
        };
    }
}
...
// Функция сортировки таблицы (ЗАЩИЩЕНО - использует Worker)
async function sortTable(column) {
    ...
    try {
        // Сортируем через Worker
        const result = await callWorkerAPI('/api/sort-table', {
            updates: allUpdates,
...
// Расчет аналитики (ЗАЩИЩЕНО - использует Worker)
async function calculateAnalytics(data) {
    try {
        const result = await callWorkerAPI('/api/calculate-analytics', { data: data });
        ...
        if (analytics.mostActiveTable) {
            document.getElementById('mostActiveTable').textContent = t(`databases.${analytics.mostActiveTable}`);
        }
    } catch (error) {
...
// Функция пагинации для записей (ЗАЩИЩЕНО - использует Worker)
async function paginateRecords() {
    try {
        // Вместо работы с DOM, отправляем данные в Worker
        const result = await callWorkerAPI('/api/paginate-records', {
        ...
        // Скрываем все записи
        const allRecords = Array.from(document.querySelectorAll('.record-item'));
        allRecords.forEach(record => record.style.display = 'none');

        // Показываем только записи текущей страницы
        result.records.forEach(record => {
            const element = document.querySelector(`.record-item[data-id="${record.id}"]`);
            if (element) {
        ...
        updatePaginationControls('records-pagination', currentPage, result.totalPages, result.totalRecords);
    } catch (error) {
...
async function recreateCharts() {
    ...
    const typeStats = await calculateTypeStats(monitoringData.recentUpdates || []);
